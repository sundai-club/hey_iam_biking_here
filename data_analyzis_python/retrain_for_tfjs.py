#!/usr/bin/env python3
"""
Retrain models with TensorFlow.js compatibility
"""
...
def save_models_tfjs_compatible(bike_count_model, accident_models, 
    ...
    """Save models in TensorFlow.js compatible format"""
    print("Saving models in TensorFlow.js compatible format...")
    
    os.makedirs('models_tfjs_ready', exist_ok=True)
    
    # Save bike count model
    bike_count_model.save('models_tfjs_ready/bike_count_model.keras')
    
    # Save accident rate models
    for name, model in accident_models.items():
        model.save(f'models_tfjs_ready/{name}_accident_rate_model.keras')
    
    # Save preprocessors
    with open('models_tfjs_ready/intersection_encoder.pkl', 'wb') as f:
        pickle.dump(intersection_encoder, f)
    
    with open('models_tfjs_ready/weather_encoder.pkl', 'wb') as f:
        pickle.dump(weather_encoder, f)
    
    with open('models_tfjs_ready/temp_scaler.pkl', 'wb') as f:
        pickle.dump(temp_scaler, f)
    
    with open('models_tfjs_ready/feature_scaler.pkl', 'wb') as f:
        pickle.dump(feature_scaler, f)
    
    print("All models and preprocessors saved to models_tfjs_ready/ directory")
...
def convert_to_tfjs():
    ...
    for model_file, model_name in models.items():
        input_path = f"models_tfjs_ready/{model_file}"
        output_path = f"models_tfjs_ready/{model_name}_tfjs"
...
def main():
    ...
    print("=== Retraining Models for TensorFlow.js Compatibility ===")
    ...
    print("Models saved to models_tfjs_ready/ directory")
